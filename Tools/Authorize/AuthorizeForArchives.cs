...
namespace Tools.Authorize
{
    ...
    /// 客户认证
    /// </summary>
    public class AuthorizeForArchives : TypeFilterAttribute
    {
        #region 字段
        ...
        public AuthorizeForArchives(bool ignore = false) : base(typeof(AuthorizeArchivesFilter))
        {
        ...
        /// 管理员授权过滤器
        /// </summary>
        private class AuthorizeArchivesFilter : IAuthorizationFilter
        {
            #region 字段
            ...
            public AuthorizeArchivesFilter(bool ignoreFilter /*, IPermissionService permissionService*/ )
            {
            ...
            public void OnAuthorization(AuthorizationFilterContext filterContext)
            {
                ...
                var actionFilter = filterContext.ActionDescriptor.FilterDescriptors
                    ...
                    .Select(filterDescriptor => filterDescriptor.Filter).OfType<AuthorizeForArchives>().FirstOrDefault();


                if (actionFilter?.IgnoreFilter ?? _ignoreFilter)
                    return;

                if (filterContext.Filters.Any(filter => filter is AuthorizeArchivesFilter))
                {
